...
for(j in 1:length(rownames(result))){
  result$AGTR1[j] <- ifelse(result$BP[j] %in% data[[1]][,1], 1, 0)
  result$CXCL12[j] <- ifelse(result$BP[j] %in% data[[2]][,1], 1, 0)
  result$PDGFRL[j] <- ifelse(result$BP[j] %in% data[[3]][,1], 1, 0)
  result$PTGER3[j] <- ifelse(result$BP[j] %in% data[[4]][,1], 1, 0)
  result$S1PR1[j] <- ifelse(result$BP[j] %in% data[[5]][,1], 1, 0)
}

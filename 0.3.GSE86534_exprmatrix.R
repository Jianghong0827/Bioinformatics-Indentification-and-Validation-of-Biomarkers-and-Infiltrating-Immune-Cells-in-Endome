#---------------------20210918
setwd("F:/gxe_data/YQ180-8/0.download data/GSE86534")

#---------------------loading packages
library(GEOquery)

#---------------------download data
gse <- getGEO("GSE86534", destdir = ".", AnnotGPL = F, getGPL = F)  #表达矩阵
gset <- exprs(gse[[1]])
...
pdata <- pdata[pdata$`tissue:ch1` != "lesion adjacent tissue",]
pdata$group <- ifelse(pdata$`tissue:ch1` == "ectopic endometrium", "EC", "EU")
...
write.table(gset, "mRNA_expr_GSE86534.txt", quote = F, row.names = F, sep = "\t")
